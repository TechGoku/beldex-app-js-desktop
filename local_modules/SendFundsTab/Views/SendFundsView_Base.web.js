...
class SendFundsView extends View {
  ...
  _setup_form_amountInputLayer (tr) {
    ...
    {
      const tooltipText = 'Based on Monero network<br/>fee estimate (not final).<br/><br/>MyBeldex does not charge<br/>a transfer service fee.'
      const view = commonComponents_tooltips.New_TooltipSpawningButtonView(tooltipText, self.context)
      const layer = view.layer
      breakingDiv.appendChild(layer)
    }
    ...
    img.addEventListener(
      ...
      function () {
        ...
        if (!code || !code.location) {
          self.validationMessageLayer.SetValidationError('Beldex was unable to find a QR code in that image.')
          return
        }
        const stringData = code.data
        if (!stringData) {
          self.validationMessageLayer.SetValidationError('Beldex was unable to decode a QR code from that image.')
          return
        }
        if (typeof stringData !== 'string') {
          self.validationMessageLayer.SetValidationError('Beldex was able to decode QR code but got unrecognized result.')
          return
        }
